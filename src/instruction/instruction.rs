...
const GENERATED_INSTRUCTIONS: Dir = include_dir!("$CARGO_MANIFEST_DIR/src/instruction-gen");
...
pub fn get_generated_instructions() -> HashMap<u16, Vec<u8>> {
    ...
    for i in 0x0..=0xFF_FF {
        ...
        let micro_ops_for_i: Vec<u8> = instruction_tables.iter().nth(i).unwrap().clone().iter().rev().collect();
